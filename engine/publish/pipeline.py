...
# release/、workflows/ 本身仍放在專案根目錄（跟 characters/／camera/／prompts/
# 一樣是資料/設定，不隨程式碼一起搬進 engine/），因此預設路徑往上推三層
# （engine/publish/pipeline.py -> engine/publish -> engine -> 專案根目錄）
# 才是專案根目錄。
_PROJECT_ROOT = Path(__file__).parent.parent.parent
RELEASE_DIR = _PROJECT_ROOT / "release"
WORKFLOW_PATH = _PROJECT_ROOT / "workflows" / "flux_schnell_basic.json"

# 與 main.py 的 ACTIVE_* 常數各自獨立：main.py 是單一故事（Genesis_001）的
# 既有手動流程，不因為這次新增 Batch Pipeline 而改變；這裡是給「一次處理
# stories/ 底下所有故事」用的 Publishing Factory 入口，兩者互不影響。
...
# uploader.py 的 YouTubeUploadProvider docstring），改成 "youtube" 前請確認
# youtube_client_secrets.json 已就緒。
