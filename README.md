...
- **PromptBuilder**（`engine/prompt/builder.py`）：把一個 scene 的資料組成分層 prompt，依優先順序疊 Character（CharacterManager，每個角色各自成為獨立分類，見下方 Character Priority）→ Environment（scene 自己的 `image_prompt`）→ Lighting → Composition → Style（透過 PromptLibrary 讀取，預設用 manifest 登記的 preset），並產生獨立的 Negative prompt。PromptBuilder 本身不保存任何固定 Prompt 字串或分類設定，只負責組裝與呼叫 PromptOptimizer。任何一層模板缺席都只讓那一層變空字串、不影響其餘內容。

  **Character Priority 機制**（v0.9 Token Allocation Sprint 新增）：每個角色依「這個 scene 裡的重要程度」分成三個 tier——`main`（保護分類，永遠不裁剪，跟 Environment 同等級）、`secondary`（可裁剪，權重預設 3，跟 Lighting 同量級）、`background`（可裁剪，權重預設 1，跟 Style 同量級；權重定義在 `prompts/manifest.json` 的 `character_priority_weights`）。tier 判斷順序：先看 scene 是否有 `scene["character_priority"]`（`{character_id: tier}`，可只覆寫部分角色），沒指定的角色再套用預設規則——`scene["characters"]` 第一個角色預設 `main`，其餘預設 `secondary`（單角色 scene 因此完全等同「整個 Character 都保護」的舊行為，不需要改任何既有 story JSON 就相容）。`resolve_character_priorities(scene)` 可以查詢目前的判斷結果。

- **PromptOptimizer**（`engine/prompt/optimizer.py`）：PromptBuilder 內部使用的最佳化元件。(1) 去重：跨分類比對逗號分隔的片語，重複片語只保留第一次出現的位置；(2) Priority Token Budget：用空白斷詞數近似 token 數，超過預算（預設 77，對應 CLIP-L 上限）時依呼叫端傳入的 `protected` 集合與 `category_weights` 把剩餘預算瀑布式分配給每個可裁剪分類——**Environment 與 Main 角色永遠是保護分類、不會被裁剪**，Secondary／Background 角色與 Lighting／Composition／Style 全部依各自權重（預設 Lighting:Composition:Style = 3:2:1，Secondary/Background 角色 = 3:1）公平分配剩餘預算，而不是像 v0.9 Token Allocation Sprint 之前那樣「整個 Character 固定保護、Lighting/Composition/Style 固定挨刀」；每一次去重／裁剪都會記錄成 debug log，可從 `image/prompts/scene{N}_final_prompt.txt` 與 `prompt_report.txt` 檢視。PromptOptimizer 本身不知道「Character」「Main/Secondary/Background」這些概念，只認得呼叫端傳入的分類名稱——Character Priority 完全是 PromptBuilder 那一層的邏輯，`allocate_budget()`／`enforce_length_budget()` 都支援傳入呼叫端專屬的 `category_weights`（省略時沿用建構時的預設值）。
...
2. 依現有的 `stories/Genesis_001.json` 格式撰寫：`episode`/`book`/`chapter`/`title_zh`/`title_en`/`duration`/`scenes`（每個 scene 至少要有 `scene_number`、`image_prompt`、`narration_zh`；`characters`/`camera` 為可選欄位，缺席時效果分別等同「沒有角色」「靜態鏡頭」；`character_priority`——`{character_id: "main"/"secondary"/"background"}`——也是可選欄位，省略時預設 `characters` 陣列第一個角色是 `main`、其餘是 `secondary`，只有角色多、Token 預算吃緊的 scene 才需要手動覆寫，見第 4 節 PromptBuilder 的 Character Priority 機制）。
...
- **v0.9 — Character Priority 自動判斷**：目前多角色 scene 要達到最佳 Token 分配，仍需要人工在 story JSON 加 `character_priority` 覆寫（例如 `Genesis_001` scene 9／11）；可評估是否要讓 `StoryGenerator`／`LLMProvider` 生成故事時就自動判斷每個角色在該 scene 的重要程度並填入這個欄位，而不是都要人工事後調整。
