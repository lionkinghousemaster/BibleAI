...
- **v0.9 — camera_shot 與影片鏡頭運動整合**：Director 目前會依語意判斷 `camera_shot`（對應 `camera/*.json` 的鏡頭運動 preset），但只出現在 `prompt_report.txt` 供分析，不會覆蓋 `scene["camera"]` 或影響 `engine.video` 的 zoompan 濾鏡選擇（刻意的範圍界線，避免這次 Sprint 影響既有影片邏輯）；可評估是否讓 StoryGenerator／batch_pipeline 在 scene 沒有明確指定 `camera` 時，改用 Director 的建議值，而不是固定退回 `static`。
