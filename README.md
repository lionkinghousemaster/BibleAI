...
- **v0.9 — 多角色 scene 的 Token 上限**：`Genesis_001` 的 scene 9／11（god+adam+eve 三個角色同時出場）protected 分類（character+environment）本身就超過 77 token 上限，lighting/composition/style 會被完全裁掉——這不是資料清理能解決的問題，需要評估是否要壓縮多角色場景的角色描述，或針對這類 scene 放寬 token 上限。
